				...
				<div v-for="(book, index) in books" :key="index" class="tbl-row">
					...
					<div>
						<template v-for="(author, authorIndex) in book.authors">
							<a :href="author.link" target="_blank">{{ author.name }}</a><span v-if="authorIndex + 1 < book.authors.length" :key="authorIndex">, </span>
						</template>
					</div>
					<div>
						<template v-for="(purchaseLink, purchaseLinkIndex) in book.purchaseLinks">
							<a :href="purchaseLink.link" target="_blank">{{ purchaseLink.title }}</a><span v-if="purchaseLinkIndex + 1 < book.purchaseLinks.length" :key="purchaseLinkIndex"> , </span>
						</template>
					</div>
				</div>
			</div>
